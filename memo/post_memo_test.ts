...
import { postMemo } from "./post_memo.ts";
...
Deno.test("postMemo", async (t) => {
  const domain = "domain";
  const token = "token";
  await t.step("post memo and return the memo", async () => {
    await using baseUrl = startDummyServer(async (req, _info) => {
      assertEquals(req.method, "POST");
      assertEquals(new URL(req.url).pathname, `/teams/${domain}/posts`);
    ...
    const result = await postMemo(client, {
      title: "メモのタイトル",
      body: "メモの本文",
      draft: false,
      tags: ["rails", "ruby"],
